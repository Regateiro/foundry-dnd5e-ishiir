...
import { assert, assertApprox, collectFailures, formatFailures } from "./tests.mjs";

/**
 * Run all infrastructure extended tests.
 * @returns {Promise<object>} Test results grouped by suite.
 */
export async function runExtendedInfrastructureTests() {
  console.debug("Running infrastructure extended tests...");

  return {
    assertHelper: await test_assertHelper(),
    assertApproxHelper: await test_assertApproxHelper(),
    collectFailuresDeepNesting: await test_collectFailuresDeepNesting(),
    formatFailuresEmpty: await test_formatFailuresEmpty()
  };
}

/* ============================================ */
/*  assert Helper Edge Cases                    */
/* ============================================ */
...
/* ============================================ */
/*  assertApprox Helper Edge Cases              */
/* ============================================ */
...
async function test_assertApproxHelper() {
  console.debug(" assertApproxHelper");
  const results = {};
  ...
  // Test 11-12: Custom tolerance (wider) → passed even beyond default
  results.custom_wide_tol = assert(true, assertApprox(5, 5.1, 0.2).passed);

  // Test 13-14: Custom tolerance (narrower) → failed within default but not custom
  results.custom_narrow_tol = assert(false, assertApprox(5, 5.0006, 0.0001).passed);

  // Test 15-16: Zero values with large difference → failed
...
async function test_collectFailuresDeepNesting() {
  console.debug(" collectFailuresDeepNesting");
  const results = {};
  ...
  try {
    collectFailures(mixedStructure);
  } catch (e) {
    threwError = true;
  }
...
async function test_formatFailuresEmpty() {
  console.debug(" formatFailuresEmpty");
  const results = {};
